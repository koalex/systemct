...
module.exports = async ctx => {
    ...
    await passport.authenticate('local', async (err, user, info, status) => {
        ...
        if (!user) ctx.throw(400, info ? (info.field && !Array.isArray(info)) ? { message: [info] } : info.message ? info.message: null : null);
        ...
        ctx.body = { access_token: token, user/*user_id: user._id*/ };
