...
stats.forEach(stat => {
    if (fs.lstatSync(`${config.projectRoot}/handlers/${stat}`).isDirectory()) {
        try {
            let router = require(`${config.projectRoot}/handlers/${stat}/socket.js`);
            if (Array.isArray(router)) {
                router.forEach(handler => {
                    if (typeof handler === 'function') {
                        socketRoutes.push(handler)
                    }
                });
...
function Socket (server) {
    ...
    let apiNsp = io.of('/api');

    apiNsp.use((socket, next) => {
        let handshakeData = socket.request; // http request
        const cookies     = new Cookies(handshakeData, {}, config.secret);
        let locale;
